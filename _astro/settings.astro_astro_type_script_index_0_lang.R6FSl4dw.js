import{a as h,t as E}from"./api.CcTKbNwm.js";let T={},w=!1;const F=[{value:"EC",label:"Établissement de crédit"},{value:"PSP",label:"Prestataire de services de paiement"},{value:"EME",label:"Établissement de monnaie électronique"},{value:"SGP",label:"Société de gestion de portefeuille"},{value:"CASP",label:"Prestataire de services sur actifs numériques"},{value:"CIF",label:"Conseiller en investissement financier"},{value:"IFP",label:"Intermédiaire en financement participatif"},{value:"OTHER",label:"Autre"}],$=[{code:"psp_versement_retrait",label:"Versement / retrait d'espèces sur compte de paiement"},{code:"psp_prelevement",label:"Prélèvement"},{code:"psp_paiement_carte",label:"Paiement par carte"},{code:"psp_virement",label:"Virement"},{code:"psp_emission_instrument",label:"Émission d'instrument de paiement"},{code:"psp_acquisition",label:"Acquisition d'opérations de paiement"},{code:"psp_transmission_fonds",label:"Transmission de fonds"},{code:"psp_initiation",label:"Initiation de paiement (DSP2)"},{code:"psp_information_comptes",label:"Information sur les comptes (DSP2)"}],D=[{code:"casp_conservation",label:"Conservation pour compte de tiers d'actifs numériques"},{code:"casp_achat_vente",label:"Achat / vente d'actifs numériques contre monnaie légale"},{code:"casp_echange",label:"Échange d'actifs numériques contre d'autres actifs numériques"},{code:"casp_plateforme",label:"Exploitation d'une plateforme de négociation"}],O=[{code:"casp_rto",label:"Réception et transmission d'ordres pour compte de tiers"},{code:"casp_gestion_portefeuille",label:"Gestion de portefeuille d'actifs numériques pour compte de tiers"},{code:"casp_conseil",label:"Conseil aux souscripteurs d'actifs numériques"},{code:"casp_prise_ferme",label:"Prise ferme / placement garanti / placement non garanti"}],k={PSP:["LCB-FT","DSP2","DORA"],EME:["LCB-FT","DSP2","DORA"],Banque:["LCB-FT","DORA","AMLR"],CASP:["LCB-FT","MiCA","AMLR"]};function S(e,i){if(!e)return"";let t='<label style="display:block;font-size:12px;font-weight:500;color:var(--ink);margin-bottom:4px;font-family:var(--font-mono);text-transform:uppercase;letter-spacing:0.1em">Services fournis</label>';if(e==="Banque"){const s=i.includes("banque_credit")||!i.length,o=i.includes("banque_correspondance");return t+=`
        <div class="toggle-row" style="margin:4px 0">
          <div><div class="toggle-label">Activité de crédit</div></div>
        ...
        <div class="svc-legal-ref">Les opérations de banque sont présumées pour tout établissement de crédit agréé — Article L.311-1 CMF</div>
      `,t}return e==="PSP"?(t+=$.map(s=>`<label class="svc-check"><input type="checkbox" class="svc-cb" data-svc="${s.code}" ${i.includes(s.code)?"checked":""}> ${s.label}</label>`).join(""),t+='<div class="svc-legal-ref">Article L.314-1 II CMF</div>',t):e==="EME"?(t+='<label class="svc-check disabled"><input type="checkbox" class="svc-cb" data-svc="eme_emission_gestion" checked disabled> Émission et gestion de monnaie électronique</label>',t+='<hr style="margin:8px 0;border:none;border-top:0.5px solid var(--line)">',t+=$.map(s=>`<label class="svc-check"><input type="checkbox" class="svc-cb" data-svc="${s.code}" ${i.includes(s.code)?"checked":""}> ${s.label}</label>`).join(""),t+='<div class="svc-legal-ref">Articles L.315-1 et L.526-1 CMF</div>',t):e==="CASP"?(t+='<div class="svc-group-title">Services nécessitant un enregistrement obligatoire (1° à 4°)</div>',t+=D.map(s=>`<label class="svc-check"><input type="checkbox" class="svc-cb" data-svc="${s.code}" ${i.includes(s.code)?"checked":""}> ${s.label}</label>`).join(""),t+='<div class="svc-separator"></div>',t+='<div class="svc-group-title">Services nécessitant un agrément optionnel (5°)</div>',t+=O.map(s=>`<label class="svc-check"><input type="checkbox" class="svc-cb" data-svc="${s.code}" ${i.includes(s.code)?"checked":""}> ${s.label}</label>`).join(""),t+='<div class="svc-legal-ref">Article L.54-10-2 CMF</div>',t):""}const N=["LCB-FT","DORA","DSP2","MiCA","AI Act","AMLR","PSD3","RGPD"],j=["critical","high","medium","low"],G={critical:"Critique",high:"Élevé",medium:"Moyen",low:"Faible"};async function U(){try{const e=await h("/api/v1/settings");T=e,_(e)}catch(e){document.getElementById("content").innerHTML=`<div class="st-loading">Erreur : ${e.message}</div>`}}function _(e){const i=e.tenant||{},t=e.settings||{},s=e.profile||{},o=e.users||[],n=e.counts||{},c=F.map(l=>`<option value="${l.value}" ${l.value===(t.entity_type||"")?"selected":""}>${l.label}</option>`).join(""),m=N.map(l=>`<button class="st-chip ${(t.regulations||[]).includes(l)?"active":""}" data-reg="${l}">${l}</button>`).join(""),r=j.map(l=>`<button class="st-chip ${(t.alert_severities||["critical","high"]).includes(l)?"active":""}" data-sev="${l}">${G[l]}</button>`).join(""),g=t.profil||{},u=t.conformite||{},a=t.outils||{},p=t.perimetre||{},B=t.organisation||{},d=g.entity_type||t.entity_type||"",x=g.nom_etablissement||B.nom_etablissement||i.name||"",A=["Particuliers","Entreprises","Institutionnels"],q=["France uniquement","Union européenne","International"],C=A.map(l=>`<button class="st-chip ${(p.types_clients||[]).includes(l)?"active":""}" data-client="${l}">${l}</button>`).join(""),P=q.map(l=>`<button class="st-chip ${(p.zones_geographiques||[]).includes(l)?"active":""}" data-zone="${l}">${l}</button>`).join(""),L=i.plan==="pro"?"st-plan-pro":i.plan==="starter"?"st-plan-starter":"st-plan-demo",R=o.map(l=>{const b=l.role==="admin"?"role-admin":l.role==="compliance_officer"?"role-compliance_officer":"role-user",M=l.last_login_at?new Date(l.last_login_at).toLocaleDateString("fr-FR",{day:"numeric",month:"short",year:"numeric"}):"Jamais";return`<tr>
      ...
      <div class="st-section" id="wizard-org">
        ...
        <div class="st-section-body">
          ...
          <div class="wiz-step-label" id="wiz-step-label">Étape 1 — Établissement</div>

          <!-- Step 1: Etablissement -->
          <div class="wiz-step" id="wiz-step-1">
            <div class="st-form-group" style="margin-bottom:16px">
              <label>Nom de l'établissement *</label>
              <input type="text" id="f-profil-nom" value="${x}" required>
            </div>
            <div class="st-form-group">
              <label>Type d'entité *</label>
          ...
          <div class="wiz-step" id="wiz-step-2" style="display:none">
            ...
            <div class="st-form-row">
              <div class="st-form-group"><label>Suppléant — Nom</label><input type="text" id="f-conf-suppleant" value="${u.responsable_lcbft_suppleant||""}"></div>
            </div>
            <div class="toggle-row" style="margin:12px 0">
              <div><div class="toggle-label">Le déclarant Tracfin est le responsable LCB-FT</div></div>
            ...
            <div id="declarant-fields" style="${u.declarant_est_responsable?"display:none":""}">
              <div class="st-form-row">
                <div class="st-form-group"><label>Déclarant Tracfin — Nom *</label><input type="text" id="f-conf-tracfin-nom" value="${u.declarant_tracfin_nom||""}"></div>
                <div class="st-form-group"><label>Déclarant Tracfin — Fonction *</label><input type="text" id="f-conf-tracfin-fn" value="${u.declarant_tracfin_fonction||""}"></div>
              </div>
            </div>
            <div class="st-form-group" style="margin-top:12px">
              <label>Contrôle périodique *</label>
              <div class="wiz-radios" id="wiz-controle-radios">
                <label class="wiz-radio ${(u.controle_periodique||"interne")==="interne"?"selected":""}"><input type="radio" name="wiz-controle" value="interne" ${(u.controle_periodique||"interne")==="interne"?"checked":""}> Internalisé</label>
                <label class="wiz-radio ${u.controle_periodique==="externe"?"selected":""}"><input type="radio" name="wiz-controle" value="externe" ${u.controle_periodique==="externe"?"checked":""}> Externalisé</label>
              </div>
            </div>
          ...
          <div class="wiz-step" id="wiz-step-3" style="display:none">
            ...
            <div class="st-form-group" style="margin-bottom:12px">
              ...
              <div class="wiz-radios" id="wiz-monitoring-radios">
                <label class="wiz-radio ${(a.monitoring||"interne")==="interne"?"selected":""}"><input type="radio" name="wiz-monitoring" value="interne" ${(a.monitoring||"interne")==="interne"?"checked":""}> Interne (pas d'outil dédié)</label>
                <label class="wiz-radio ${a.monitoring==="outil"?"selected":""}"><input type="radio" name="wiz-monitoring" value="outil" ${a.monitoring==="outil"?"checked":""}> Outil dédié</label>
              </div>
            </div>
            ...
            <div class="st-form-group" style="margin-top:12px">
              ...
              <select id="f-outils-signalement">
                <option value="">— Choisir —</option>
                <option value="email"     ${a.signalement_interne==="email"?"selected":""}>Email dédié conformité</option>
                <option value="ticketing" ${a.signalement_interne==="ticketing"?"selected":""}>Outil de ticketing</option>
                <option value="direct"    ${a.signalement_interne==="direct"?"selected":""}>Remontée directe au responsable LCB-FT</option>
              </select>
            </div>
            <!-- Blockchain analytics — CASP only -->
            <div id="blockchain-section" style="${d==="CASP"?"":"display:none"}">
              <hr style="margin:16px 0;border:none;border-top:0.5px solid var(--line)">
              <div class="st-form-row">
                <div class="st-form-group">
              ...
              <div class="st-form-group">
                <label>Seuil d'alerte (défaut : 75)</label>
                <input type="number" id="f-outils-blockchain-seuil" min="0" max="100" value="${a.blockchain_seuil??75}">
                <div style="font-size:11px;color:var(--mute);margin-top:4px">La valeur standard réglementaire est 75. Ne modifier que si votre outil applique une échelle différente.</div>
              </div>
            </div>
          </div>
          ...
          <div class="wiz-step" id="wiz-step-4" style="display:none">
            ...
            <div class="st-form-group" style="margin-top:12px">
              <label>Canal d'entrée en relation *</label>
              <div class="wiz-radios" id="wiz-canal-radios">
                ...
                <label class="wiz-radio ${p.canal_entree_relation==="presentiel"?"selected":""}"><input type="radio" name="wiz-canal" value="presentiel" ${p.canal_entree_relation==="presentiel"?"checked":""}> Présentiel</label>
              </div>
            </div>
          ...
          <!-- Navigation buttons -->
          <div class="wiz-nav">
            <button class="wiz-btn wiz-btn-prev" id="wiz-prev" style="display:none">Précédent</button>
      ...
      <div class="st-section">
        <div class="st-section-header">Préférences d'alertes</div>
        <div class="st-section-body">
          <div class="toggle-row">
            <div>
              <div class="toggle-label">Alertes email</div>
              <div class="toggle-desc">Recevoir un email pour chaque reminder selon la sévérité sélectionnée</div>
            </div>
            <label class="toggle">
          ...
          <div style="margin-top:12px">
            <label style="display:block;font-size:12px;font-weight:500;color:var(--ink);margin-bottom:4px;font-family:var(--font-mono);text-transform:uppercase;letter-spacing:0.1em">Sévérités déclenchant un email</label>
          ...
          <div class="st-form-row" style="margin-top:16px">
            <div class="st-form-group">
              ...
              <select id="f-digest">
                <option value="immediate" ${t.alert_digest==="immediate"?"selected":""}>Immédiat</option>
      ...
      <div class="st-section">
        ...
        <div class="st-section-body">
          <div class="toggle-row">
            <div><div class="toggle-label">Procédures LCB-FT</div><div class="toggle-desc">${n.procedures_count||0} procédures attribuées</div></div>
            <label class="toggle"><input type="checkbox" id="f-mod-procs" ${t.procedures_enabled?"checked":""}><span class="toggle-slider"></span></label>
          </div>
          <div class="toggle-row">
            <div><div class="toggle-label">Pays à risque</div><div class="toggle-desc">Liste FATF / EU / ACPR avec croisement veille</div></div>
          ...
          <div class="toggle-row">
            <div><div class="toggle-label">Roadmap réglementaire</div><div class="toggle-desc">Échéances DORA, AMLR, AI Act</div></div>
            <label class="toggle"><input type="checkbox" id="f-mod-roadmap" ${t.roadmap_enabled!==!1?"checked":""}><span class="toggle-slider"></span></label>
          </div>
          <div class="toggle-row">
            <div><div class="toggle-label">Fiches réflexes</div><div class="toggle-desc">Guides opérationnels par situation</div></div>
      ...
      <div class="st-section">
        ...
        <div class="st-section-body">
          ...
          <div class="st-form-row">
            ...
            <div class="st-form-group">
              <label>Rôle</label>
              <input type="text" value="${s.role||""}" disabled>
            </div>
          </div>
        </div>
      </div>
