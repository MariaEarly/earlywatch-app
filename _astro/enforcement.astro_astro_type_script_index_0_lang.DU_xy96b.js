import{a as d}from"./api.CcTKbNwm.js";let o=[];async function p(){try{const[s,t]=await Promise.allSettled([d("/enforcement/stats"),d("/enforcement?limit=100")]);if(s.status==="fulfilled"){const e=s.value;document.getElementById("stat-total").textContent=e.total||0,document.getElementById("stat-critical").textContent=e.critical||0,document.getElementById("stat-high").textContent=e.high||0,document.getElementById("stat-7d").textContent=e.last_7d||0,document.getElementById("stat-30d").textContent=e.last_30d||0;const a=document.getElementById("filter-source");e.by_source&&Object.entries(e.by_source).sort((l,i)=>i[1]-l[1]).forEach(([l,i])=>{const n=document.createElement("option");n.value=l,n.textContent=`${l} (${i})`,a.appendChild(n)})}t.status==="fulfilled"&&(o=t.value.items||[],c(o))}catch(s){console.error(s)}}function c(s){const t=document.getElementById("enf-body");if(!s.length){t.innerHTML='<tr><td colspan="5" class="empty-state">Aucune sanction trouvée</td></tr>';return}t.innerHTML=s.map(e=>{const a=e.published_at?new Date(e.published_at).toLocaleDateString("fr-FR"):"-",l=`sev-${e.severity||"low"}`,i=(e.impacted_procedures||[]).map(m=>`<span class="proc-tag">${m}</span>`).join(""),n=(e.summary||"").substring(0,120);return`<tr data-show-detail="${e.id}" style="cursor:pointer">
      ...
      <div class="detail-meta">
        ...
        <span style="margin-left:12px;color:var(--mute)">${e}</span>
      </div>
      <div class="detail-summary">${t.summary||""}</div>
      ${i?`<div class="detail-section"><h4>Obligations citées</h4><ul style="padding-left:16px;font-size:13px;line-height:1.8;color:var(--taupe)">${i}</ul></div>`:""}
      ${l?`<div class="detail-section"><h4>Procédures impactées</h4><div>${l}</div></div>`:""}
